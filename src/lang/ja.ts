const root = ({
  ...
  "selector-extension-error": "拡張子は <var>.json</var> である必要があります（<var>.$2</var>）: $1",
  "selector-noextension-error": "拡張子は <var>.json</var> である必要があります: $1",
  "selector-contentmodel-error": "コンテンツモデルは <var>$3</var> である必要があります（<var>$2</var>）: $1",
  "selector-namespace-error": "名前空間は <var>Module</var> か <var>MediaWiki</var> である必要があります: $1",
  ...
  "settings-stylesheet": "スタイルシート",
  "settings-image-description": "スプライトシートとして利用する画像を、URLかファイル名で指定する（名前空間 <var>File:</var> は不要）。",
  "settings-size-description": "各スプライトの高さと横幅を指定します。この値は <var>width</var> と <var>height</var> により上書きされます。",
  "settings-width-description": "各スプライトの横幅を指定します。この値は <var>size</var> を上書きします。",
  "settings-height-description": "各スプライトの高さを指定します。この値は <var>size</var> を上書きします。",
  "settings-classname-description": "スプライトに共通で設定されるHTMLクラスを指定します（例: <code>inv-sprite</code>）。",
  "settings-stylesheet-description": "現在のデフォルト設定オブジェクトのクローンを新たに生成する。",
